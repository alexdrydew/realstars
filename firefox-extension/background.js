...
async function loadKnownFarmsBlocklist() {
  ...
  try {
    const result = await extensionApi.storage.local.get("knownFarmsBlocklist");
  ...
  try {
    const resp = await fetch(extensionApi.runtime.getURL("known-farms.json"));
...
async function getStoredToken() {
  try {
    const local = await extensionApi.storage.local.get("githubToken");
  ...
  try {
    const legacy = await extensionApi.storage.sync.get("githubToken");
    if (legacy.githubToken) {
      try {
        await extensionApi.storage.local.set({ githubToken: legacy.githubToken });
        await extensionApi.storage.sync.remove("githubToken");
      } catch {
...
extensionApi.runtime.onMessage.addListener((msg) => {
  if (msg.type === "ANALYZE_REPO") {
    return handleAnalyze(msg.owner, msg.repo, msg.pageData)
  ...
  if (msg.type === "SET_TOKEN") {
    return extensionApi.storage.local.set({ githubToken: msg.token })
      .then(() => extensionApi.storage.sync.remove("githubToken"))
      .then(() => ({ ok: true }))
  ...
  if (msg.type === "DELETE_TOKEN") {
    return Promise.allSettled([
      extensionApi.storage.local.remove("githubToken"),
      extensionApi.storage.sync.remove("githubToken"),
    ])
      .then(() => ({ ok: true }))
      .catch((err) => ({ ok: false, error: err.message }));
  }
  ...
  if (msg.type === "VALIDATE_TOKEN") {
    return validateToken(msg.token)
      .catch((err) => ({ valid: false, error: err.message }));
  }

  return Promise.resolve({ error: "unknown_message_type" });
});
